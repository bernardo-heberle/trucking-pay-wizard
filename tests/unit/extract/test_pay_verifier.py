...
class TestBasicMatches:
    ...
    def test_match_among_multiple_amounts(self) -> None:
        ...
        matched, _ = verify_pay_against_ocr("1200.50", ocr)
        assert matched is True
...
class TestNoMatch:
    ...
    def test_value_absent_returns_false(self) -> None:
        matched, _ = verify_pay_against_ocr("9999.00", "Payment: $750.00")
        assert matched is False

    def test_empty_ocr_text_returns_false(self) -> None:
        matched, _ = verify_pay_against_ocr("750.00", "")
        assert matched is False

    def test_ocr_with_no_numbers_returns_false(self) -> None:
        matched, _ = verify_pay_against_ocr("750.00", "No amounts here at all.")
        assert matched is False
